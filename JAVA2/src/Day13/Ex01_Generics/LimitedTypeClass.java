...
public class LimitedTypeClass {
	public static void main(String[] args) {
		...
		// X 클래스 타입 매개변수 지정이 불가하다
		// A,B,C 클래스는 타입 매개변수로 지정 가능하다.
		...
		// d2<B>, d3<c> <-- 부모 객체인 A대입 불가
		// 하려면 다운캐스팅을 해서 강제 형변환 필요 d2.setT((b)new A());
		// (에러) d2.setT(new A());  
		// (에러) d3.setT(new A());
		
		// * 타입 지정을 생략한 경우,
		// 제네릭 타입을 제한한 A클래스를 기본으로 타입 지정하여 생성한다.
		...
		// 그냥 쓴 정수는 (127)은 int 타입으로 인식 int 타입은 4byte
