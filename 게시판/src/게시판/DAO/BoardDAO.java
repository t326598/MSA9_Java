 ...
 */
	public Board select(int no) {
		...
		try {
			...
			if(rs.next()) {
				...
				board.setWriter(rs.getString("writer"));
				board.setContent(rs.getString("content"));
				board.setRegDate(rs.getTime("reg_date"));
				board.setUpdDate(rs.getTime("upd_date"));
			}
	...
	public int insert(Board board) {
		...
		try {
				...
				// executeUpdate() : 
				// SQL(INSERT, UPDATE, DELETE) SQL 실행 시 적용된 데이터 개수를 받아온다.
				// ex) 게시글 1개 적용 성공 시, result : 1
				//     				실패 시, result : 0
				
		} catch (SQLException e) {
			System.err.println("게시글 등록 시, 예와 발생");
			e.printStackTrace();
 ...
 */
	...
	try {
		...
		System.err.println("게시글 수정 시, 예와 발생");
		e.printStackTrace();
	...
	try {
		...
		System.err.println("게시글 삭제 시, 예와 발생");
		e.printStackTrace();
