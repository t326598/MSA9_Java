...
public class MainController {
   ...
   void setTimer() {
    	// 애니메이션 적용
    	// * 0.05 초 마다 메뉴를 바꾸다가 3초가 되면 멈춰서도록 애니메이션 적용
    	
    	// Timeline 주기적으로 처리할 수 있도록 하는 애니메이션 클래스
    	timeline = new Timeline(
    			// KeyFrame 객체 지정한 시간마다 익명함수 호출
    			// * Duration.millis(밀리초) - 여기서는 0.05초
    			new KeyFrame(Duration.millis(50), e -> 
    			{ updateMenu();  //랜덤 메뉴 갱신
    			})   
    			);
    	// 애니메이션 반복 횟수 설정 (무한 반복)
   ...
   void buttonClick(ActionEvent event) {
    ...
    public void updateMenu() {
    	 try {
    	    	...
    	    	String filePath = getClass().getResource("/img/icon.png").toExternalForm();
    	    	img.setImage(new Image(filePath));
    	    }
    }
